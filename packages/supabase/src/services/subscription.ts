...
export class SubscriptionService {
   ...
   */
  async getUserSubscription(userId: string): Promise<UserSubscriptionWithTier | null> {
    const { data, error } = await this.supabase
   ...
   */
  async getUserSubscriptionEvents(userId: string, limit: number = 10): Promise<SubscriptionEvent[]> {
    const { data, error } = await this.supabase
  ...
  async getSubscriptionStats(): Promise<{
    ...
    const active_subscriptions =
      allSubs?.filter((s: any) => s.status === 'active' || s.status === 'trialing').length || 0;
    const premium_subscriptions =
      allSubs?.filter(
        (s: any) =>
          (s.status === 'active' || s.status === 'trialing') && s.tier?.price_monthly > 0
      ).length || 0;
    ...
    const mrr =
      allSubs
        ...
        .reduce((sum: number, s: any) => sum + (s.tier?.price_monthly || 0), 0) || 0;
