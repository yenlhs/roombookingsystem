...
export const adminExtendSubscriptionSchema = z.object({
  subscriptionId: z.string().uuid('Invalid subscription ID'),
  extensionDays: z.number().int().min(1, 'Extension must be at least 1 day').max(365, 'Extension cannot exceed 365 days'),
  reason: z.string().min(10, 'Reason must be at least 10 characters').max(500, 'Reason must be at most 500 characters'),
});
...
export type AdminCreateManualSubscriptionInput = z.infer<typeof adminCreateManualSubscriptionSchema>;
...
export type AdminTransferSubscriptionInput = z.infer<typeof adminTransferSubscriptionSchema>;
...
export const adminUpdateTierSchema = z.object({
  tierId: z.string().uuid('Invalid tier ID'),
  updates: z.object({
    name: z.string().min(2).max(50).regex(/^[a-z0-9-]+$/).optional(),
