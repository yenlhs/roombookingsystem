      ...
      <View className="flex-1 bg-slate-50">
        <View className="p-6">
          ...
          <TouchableOpacity onPress={() => router.back()} className="mb-6">
            <Text className="text-blue-600 font-semibold text-base">← Back to Rooms</Text>
          </TouchableOpacity>
    ...
    <View className="flex-1 bg-slate-50">
      ...
      >
        <View className="p-6 pb-24">
          ...
          <View className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 mb-4">
            <View className="flex-row justify-between items-start mb-4">
              <View className="flex-1 mr-4">
                <Text className="text-3xl font-bold text-gray-900 mb-2">{room.name}</Text>
                {room.description && (
              ...
              <View className={`px-4 py-2 rounded-full ${getRoomStatusColor(room.status)}`}>
                <Text
            ...
            {room.status === 'inactive' && (
              <View className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                <Text className="text-sm text-yellow-800">
                  ⚠️ This room is currently inactive and not available for booking
                </Text>
              </View>
            )}
          </View>
          ...
          {/* Room Details Card */}
          <View className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 mb-4">
            <Text className="text-xl font-bold text-gray-900 mb-4">Room Details</Text>
          ...
          {room.image_urls && room.image_urls.length > 0 && (
            <View className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 mb-4">
              <Text className="text-xl font-bold text-gray-900 mb-4">Room Images</Text>
              <Text className="text-sm text-gray-600">Images will be displayed here</Text>
            </View>
          )}
          ...
          {room.status === 'active' && (
            ...
            >
              <Text className="text-white font-bold text-lg">Book This Room</Text>
            </TouchableOpacity>
          )}
