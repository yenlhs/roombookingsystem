...
async function handleCheckoutSessionCompleted(supabase: any, event: Stripe.Event) {
  ...
  await supabase.from('subscription_events').insert({
    user_id: userId,
    event_type: subscription.status === 'trialing' ? 'trial_started' : 'subscription_created',
...
async function handleSubscriptionUpdated(supabase: any, event: Stripe.Event) {
  ...
  await supabase.from('subscription_events').insert({
    ...
    event_type: subscription.cancel_at_period_end ? 'subscription_cancelled' : 'subscription_updated',
    stripe_event_id: event.id,
    metadata: {
      subscription_id: subscription.id,
      status: subscription.status,
...
async function handleSubscriptionDeleted(supabase: any, event: Stripe.Event) {
  ...
  console.log(`Subscription ${subscription.id} deleted, user reverted to free tier`);
}
